		...
		if (id.length > 0){
			  ...
			  $("#AgendaTimeStart option").filter(function() {
				  ...
				  return $(this).text() == data[1]; 
					...
					if (data[14] == 1) {
					   $('#Moderator').attr('checked', 'checked');
					   	 $('#AgendaIcon').hide();
					     $('#SpeakersDiv').show();
						 ...
						 modinput.setAttribute('name', 'ModeratorId');
						 modinput.setAttribute('id', 'ModeratorId');
						 modinput.setAttribute('type', 'hidden');
						 modinput.setAttribute('value', data[15]);
						  
						document.getElementById("agenda").appendChild(modinput);
					}
