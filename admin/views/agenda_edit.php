...
<body>
 ...
 <!--Main Wrapper-->
	<div class="wrapper">
	  <h1 class="WrapperMainH1">HR Tech Europe - London2015 |<br /> Edit Session</h1>
  ...
  // $content .= $new->agenda_list();
   ...
   // </select></label><br /><br />';
		   ...
		   if (isset($_COOKIE['AgendaEditVal'])){
		    $content .= '<input id="SecretAgendaEditId" style="display:none;" name="SecretAgendaEditId" type="text" value="'.$_COOKIE['AgendaEditVal'].'">';
	       }
	 $content .='
   ...
   $content .= ' 
	  ...
	  $content .='  <label>End<select id="AgendaTimeEnd" name="AgendaTimeEnd">';
   $content .= $new->agenda_time_list();
   $content .= ' 
    </select></label><br />
	  </fieldset>
  ...
  $content .= $new->get_speakers();
   ...
   <p>In order to select multiple speakers, please hold Ctrl and then click on the speakers</p></div>
	...
	<input name="AgendaTag" id="AgendaTag" type="hidden" />
     ...
     <input class="AdminSubmitButton" name="AgendaEditSubmit" type="submit" value="Save"/>
   </div>
  </form>
    	   <!-- End of Form Container-->
	 </div>
...
<head>
  ...
  <title>HR Tech Europe - Edit Agenda Item</title>
<?php
echo $content;
